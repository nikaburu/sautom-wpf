using System;
using System.Collections;
using System.ComponentModel;
using System.Timers;
using System.Windows;
using System.Windows.Controls;
using System.Windows.Input;

namespace Sautom.Client.Modules.Proposal.Controls
{
    /// <summary>
    /// Autocomplete User control
    /// </summary>
    public partial class AutoComplete : ComboBox
    {
	    public static readonly DependencyProperty PatternChangedProperty = DependencyProperty.Register("PatternChanged", typeof(Action<AutoCompleteArgs>), typeof(AutoComplete));

	    #region Constructor(s)

	    public AutoComplete()
        {
            InitializeComponent();

            if (!DesignerProperties.GetIsInDesignMode(this))
            {
                _interval = new Timer(Delay);
                _interval.AutoReset = true;
                _interval.Elapsed += _interval_Elapsed;
            }
        }

	    #endregion

	    public Action<AutoCompleteArgs> PatternChanged 
        {
            get { return (Action<AutoCompleteArgs>)GetValue(PatternChangedProperty); }
            set { SetValue(PatternChangedProperty, value); } 
        }

	    #region Events and Delegates

	    /// <summary>
        /// Used to pass arguments for autocomplete control
        /// </summary>
        public class AutoCompleteArgs : EventArgs
        {
	        /// <summary>
            /// default false
            /// </summary>
            private bool _cancelBinding;

		    public AutoCompleteArgs(string Pattern)
            {
                this.Pattern = Pattern;
            }

	        /// <summary>
            /// The current pattern in the auto complete
            /// </summary>
            public string Pattern { get; }

		    /// <summary>
            /// Used to the the new datasource for the autocomplete
            /// </summary>
            public IEnumerable DataSource { get; set; }

		    /// <summary>
            /// Determines weather or not the datasource should be bounded to the autocomplete control
            /// </summary>
            public bool CancelBinding
            {
                get
                {
                    return _cancelBinding;
                }
                set
                {
                    _cancelBinding = value;
                }
            }
        }

	    /// <summary>
        /// event handler for auto complete pattern changed
        /// </summary>
        /// <param name="sender"></param>
        /// <param name="args"></param>
        public delegate void AutoCompleteHandler(object sender, AutoCompleteArgs args);

	    ///// <summary>
	    ///// Event for when pattern has been changed
	    ///// </summary>
	    //public event AutoCompleteHandler PatternChanged;

	    #endregion

	    #region Fields

	    private readonly Timer _interval;
	    private bool IsKeyEvent;
	    private bool _clearOnEmpty = true;
	    private const int DEFAULT_DELAY = 800; // 1 seconds delay

	    #endregion

	    #region Properties

	    /// <summary>
        /// The timespan between keypress and pattern changed event
        /// </summary>
        [DefaultValue(DEFAULT_DELAY)]
        public int Delay { get; set; } = DEFAULT_DELAY;

	    /// <summary>
        /// The maximum number of records to show in the drop down
        /// </summary>
        public int MaxRecords { get; set; } = 10;

	    /// <summary>
        /// Determines weather textbox does type ahead
        /// </summary>
        public bool TypeAhead { get; set; } = false;

	    /// <summary>
        /// Gets the text box in charge of the editable portion of the combo box.
        /// </summary>
        protected TextBox EditableTextBox
        {
            get
            {
                return GetTemplateChild("PART_EditableTextBox") as TextBox;
            }
        }

	    /// <summary>
        /// returns the selected items text representation
        /// </summary>
        public string SelectedText
        {
            get
            {
                if (SelectedIndex == -1) return string.Empty;

                return SelectedItem.ToString();
            }
        }

	    /// <summary>
        /// Specify weather or not to clear the datasource when the text is set to empty string
        /// </summary>
        /// <remarks>When this property is set to true, the patternchanged event will not be fired when the text is empty.</remarks>
        public bool ClearOnEmpty
        {
            get
            {
                return _clearOnEmpty;
            }
            set
            {
                _clearOnEmpty = true;
            }
        }

	    #endregion

	    #region Event Handlers

	    void _interval_Elapsed(object sender, ElapsedEventArgs e)
        {
            IsKeyEvent = false;
            // pause the timer
            _interval.Stop();

            // show the drop down when user starts typing then stops
            Dispatcher.BeginInvoke((Action)delegate
            {
                IsDropDownOpen = true;
                // load from event source
                if (PatternChanged != null)
                {
                    AutoCompleteArgs args = new AutoCompleteArgs(Text);
                    PatternChanged(args);
                    // bind the new datasource if user did not cancel
                    if (!args.CancelBinding)
                    {
                        ItemsSource = args.DataSource;

                        if (!TypeAhead)
                        {
                            // override auto complete behavior
                            Text = args.Pattern;
                            EditableTextBox.CaretIndex = args.Pattern.Length;
                        }
                    }
                }
            },
                System.Windows.Threading.DispatcherPriority.ApplicationIdle);
        }

	    private void ComboBox_SelectionChanged(object sender, SelectionChangedEventArgs e)
        {
            _interval.Stop();
        }

	    public override void OnApplyTemplate()
        {
            base.OnApplyTemplate();

            //load the text box control
            if (EditableTextBox != null)
            {
                EditableTextBox.PreviewKeyDown += EditableTextBox_PreviewKeyDown;
                EditableTextBox.TextChanged += EditableTextBox_TextChanged;
            }
        }

	    void EditableTextBox_PreviewKeyDown(object sender, KeyEventArgs e)
        {
            IsKeyEvent = true;
        }

	    void EditableTextBox_TextChanged(object sender, TextChangedEventArgs e)
        {
            // clear the itemsource when text is empty
            if (ClearOnEmpty && string.IsNullOrEmpty(EditableTextBox.Text.Trim()))
                ItemsSource = null; // this should also clear selection
            else if (IsKeyEvent)
                ResetTimer();
        }

	    protected void ResetTimer()
        {
            _interval.Stop();
            _interval.Start();
        }

	    #endregion
    }
}